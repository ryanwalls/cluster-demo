...
ENV APPLICATION_NAME=cluster-demo
...
RUN mkdir -p $GOPATH/src/github.com/ryanwalls/$APPLICATION_NAME

COPY . $GOPATH/src/github.com/ryanwalls/$APPLICATION_NAME

WORKDIR $GOPATH/src/github.com/ryanwalls/$APPLICATION_NAME
RUN glide install
RUN go test $(glide novendor)
# Must run after glide installs 3rd party (vendor) dependencies.  Installs binary on path.
RUN go install

ENTRYPOINT ["cluster-demo"]
